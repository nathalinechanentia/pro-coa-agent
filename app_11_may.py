...
# ── Neo4j health check ────────────────────────────────────────────────────────
@st.cache_data(ttl=30)   # re-check at most every 30 seconds
def check_neo4j() -> str:
    try:
...
def render_kg_evidence_cards(result: dict):
    ...
    with st.expander(
        ...
        if scored:
            ...
            useful_records = [r for r in scored if _is_useful(r)]
            ci = result.get("citation_index", {})
            # Build reverse lookup: instrument_name → label from citation_index
            inst_to_label = {
                v.get("instrument", ""): k
                for k, v in ci.items()
            ...
            rows = []
            for i, inst in enumerate(useful_records[:15], 1):
                ...
                ref_label = inst_to_label.get(inst_name, f"TI-{i:03d}")
                rows.append({
                    "Ref":         ref_label,
                    ...
                    "Score":       f"{inst.get('scientific_score', 0)}/100",
                    "Role":        inst.get("endpoint_role", "") or inst.get("pro_position", ""),
                    "Key finding": kf[:100] if len(kf) > 100 else kf,
                    "Year":        inst.get("publication_year", ""),
                    "P-value":     inst.get("p_value", ""),
                    "Effect size": inst.get("effect_size", ""),
...
def audit_uncited_sentences(answer: str) -> list:
    ...
    cleaned = re.sub(
        r'\[[^\]]+\]\(https?://[^\)]+\)', '', answer)
    cleaned = re.sub(
        r'\[(TI|CT|RR|REJ|IR|RULE|PREC|COMP)-\d+\]', '', cleaned)  # KG labels
...
def _markdown_table_to_df(text: str, heading_keyword: str) -> pd.DataFrame:
    """Extract the first markdown table following a heading containing heading_keyword."""
    lines = text.split("\n")
    ...
    def parse_row(r):
        # Remove any superscript citation markers like [4] or [[5]] or <sup>...</sup>
        cell_text = re.sub(r'<sup[^>]*>.*?</sup>', '', r, flags=re.DOTALL)
        cell_text = re.sub(r'\[{1,2}\d+\]{1,2}', '', cell_text)   # [N] or [[N]]
        # Replace rogue pipes inside cells with an em-dash (should not happen per prompt)
        # but we can't reliably handle them, so fallback: if number of cells differs, skip row
...
def answer_direct(user_message: str, history: list,
    ...
    if prior_result and prior_result.get("kg_evidence_block"):
        # Case 1: follow-up — send only the scored instrument list and top records,
        # not the full evidence block. Full block is 15-20k tokens; not needed for follow-ups.
    ...
    if prior_result:
        ...
        # Get top candidates from coverage matrix (replaces old battery_result)
        top_candidates = [
            s["instrument_name"]
...
def render_strategy_result(answer: str, result: dict, msg_idx: int) -> None:
    ...
    if comparator_trials:
        with st.expander(
            ...
            drug_class = ctx.get("drug_class", ctx.get("drugclass", ""))
            st.caption(
                "Trials ranked by: (1) same drug class as current trial, "
                "(2) overlapping mechanism keywords, (3) most recent trial."
            )
            for comp in comparator_trials:
                comp_class = comp.get("drug_class", "")
                is_same = (drug_class.lower() in comp_class.lower()
                           or comp_class.lower() in drug_class.lower()) if drug_class else False
    ...
    # ───────── Download buttons for the tables ────────
    table_exports = [
    ...
    if top_scores:
        with st.expander("📊 Evidence context — instrument scoring detail (used to brief the agent)", expanded=False):
...
with st.sidebar:
    ...
    # ── Trial Context (collapsed by default) ─────────────
    ...
    if st.session_state.get("messages"):
        ...
        for i, msg in enumerate(st.session_state.messages):
            ...
            lines.append(f"\n[{role}]\n{msg['content']}")
            # Attach full result JSON for strategy answers
        ...
        st.download_button(
            ...
            mime="text/plain",
            use_container_width=True,
            key="download_conv_main",          # static key = always up‑to‑date
...
if prompt:
    ...
    with st.chat_message("assistant"):
        ...
        # ── TIER 3: Full strategy pipeline ────────────────────────────────────
            ...
            # Step 3 — only re-extract if sidebar actually added something new
            if full_prompt.strip() != prompt.strip():
                _ctx = analyze_trial_context(full_prompt)
            else:
                _ctx = _raw_ctx   # reuse — no sidebar additions, no point re-calling
